...
const DoctorDashboard = ({ doctorInfo: propDoctorInfo }) => {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1); // reset trang mỗi khi đổi ngày/tuần
  ...
  const goToPreviousDay = () => { const newDate = new Date(selectedDate); newDate.setDate(selectedDate.getDate() - 1); setSelectedDate(newDate); };
  ...
  const getAppointmentCountForDate = (date) => appointments.filter(app => isSameDay(app.appointment_time, date)).length;
  ...
  if (error) return (
    <div className="flex flex-col justify-center items-center h-96 space-y-4">
      ...
      <div className="text-center">
        ...
        <button onClick={fetchAppointments} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-all">Thử lại</button>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold mb-2">Xin chào, {doctorInfo?.profile?.full_name || 'Bác sĩ'}</h2>
            <p className="text-blue-100 text-lg">
              {isToday ? 'Hôm nay' : new Date(selectedDate).toLocaleDateString('vi-VN', { weekday: 'long', day: 'numeric', month: 'long' })} có <strong className="text-white font-bold">{stats.total}</strong> lịch hẹn
            </p>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
              <CalendarDays className="text-blue-600" size={24}/> Lịch làm việc
            </h3>
            <div className="flex gap-2">
              <button onClick={() => setViewMode('day')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode==='day'?'bg-blue-600 text-white':'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>Theo ngày</button>
              <button onClick={() => setViewMode('week')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${viewMode==='week'?'bg-blue-600 text-white':'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>Theo tuần</button>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={goToPreviousDay} className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors"><ChevronLeft size={20} /></button>
            <button onClick={goToToday} className="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors text-sm font-medium">Hôm nay</button>
            ...
            <button onClick={goToNextDay} className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors"><ChevronRight size={20} /></button>
          </div>
        </div>
        ...
        {viewMode === 'week' && (
          <div className="grid grid-cols-7 gap-3 mb-6">
            {weekDates.map((date, index) => {
              ...
              return (
                <button key={index} onClick={() => setSelectedDate(date)} className={`p-4 rounded-lg border-2 transition-all ${isSelected?'border-blue-600 bg-blue-50':isTodayDate?'border-blue-300 bg-blue-50/50':'border-gray-200 hover:border-gray-300'}`}>
                  <p className={`text-xs font-medium mb-1 ${isSelected?'text-blue-600':'text-gray-500'}`}>{date.toLocaleDateString('vi-VN',{ weekday:'short'})}</p>
                  <p className={`text-2xl font-bold mb-2 ${isSelected?'text-blue-600':'text-gray-800'}`}>{date.getDate()}</p>
                  {count>0 && <span className={`text-xs px-2 py-1 rounded-full ${isSelected?'bg-blue-600 text-white':'bg-blue-100 text-blue-600'}`}>{count} lịch</span>}
                </button>
              );
            })}
          </div>
        ...
        {filteredAppointments.length > 0 ? (
          <div>
            <h4 className="font-semibold text-gray-700 mb-4 flex items-center gap-2"><Clock size={18}/> Danh sách lịch hẹn ({filteredAppointments.length})</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {paginatedAppointments.sort((a,b)=>new Date(a.appointment_time)-new Date(b.appointment_time))
            ...
            {totalPages>1 && (
              <div className="flex justify-center items-center mt-6 gap-2">
                <button onClick={()=>goToPage(currentPage-1)} disabled={currentPage===1} className="px-3 py-1 rounded border border-gray-300 hover:bg-gray-100 disabled:opacity-50">Trước</button>
                {Array.from({length: totalPages}, (_,i)=>i+1).map(page=>(
                  <button key={page} onClick={()=>goToPage(page)} className={`px-3 py-1 rounded border ${currentPage===page?'bg-blue-600 text-white':'border-gray-300 hover:bg-gray-100'}`}>{page}</button>
                ))}
                <button onClick={()=>goToPage(currentPage+1)} disabled={currentPage===totalPages} className="px-3 py-1 rounded border border-gray-300 hover:bg-gray-100 disabled:opacity-50">Tiếp</button>
              </div>
            )}
          </div>
        ) : (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 font-medium text-lg">
              {viewMode==='day'? (isToday?'Hôm nay không có lịch hẹn nào':'Ngày này không có lịch hẹn nào')
